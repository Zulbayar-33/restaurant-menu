// Put your Google Apps Script Web App URL here after deploy
...
function renderMenu(){
  ...
  const filtered = menu.filter(f => (activeCategory === 'all' || f.category === activeCategory) && f.name.toLowerCase().includes(q));
    ...
    <div class="col-sm-6 col-lg-4">
      <div class="card food-card">
        ...
        <div class="food-body">
          ...
          <button class="btn btn-primary add-btn w-100" onclick="addToCart(${f.id})">Сагсанд нэмэх</button>
        </div>
      </div>
    </div>`).join('');
}
...
function renderCart(){
  ...
  box.innerHTML = cart.length ? cart.map(i => `
    ...
    </div>`).join('') : '<p class="text-muted text-center">Сагс хоосон байна.</p>';
}
...
async function submitOrder(){
  ...
  if(!data.customer || !data.phone || cart.length === 0){
    message.textContent = 'Нэр, утас, хоол заавал оруулна уу.';
